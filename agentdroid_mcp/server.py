        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "output_path": {
                        "type": "string",
                        "description": "Path to save UI dump",
                        "default": "ui_dump.xml",
                    },
                },
            },
...
def get_target_device(serial: str | None = None):
    ...
    try:
        ...
        if name == "record_skill":
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"Recording skill '{skill_name}' for {duration} seconds.",
                )
            ]

        if name == "replay_skill":
            replay_skill(device, arguments["name"])
            return [
                types.TextContent(type="text", text=f"Replayed skill: {arguments['name']}")
            ]
        ...
        if name == "discover_links":
            links = get_deep_links(device.serial, arguments["package_name"])
            output = (
                f"Discovered links for {arguments['package_name']}:\n" + "\n".join(links)
            )
        ...
        if name == "tap":
            ...
            return [
                types.TextContent(
                    type="text", text=f"Tapped at {arguments['x']}, {arguments['y']}"
                )
            ]
        ...
        if name == "launch_app":
            ...
            return [
                types.TextContent(
                    type="text", text=f"Launched {arguments['package_name']}"
                )
            ]

        if name == "mirror_screen":
            device.mirror()
            return [
                types.TextContent(type="text", text=f"Mirroring started for {device.serial}")
            ]
        ...
        if name == "inspect_layout":
            ...
            return [
                types.TextContent(
                    type="text", text=f"Layout bounds {status} on {device.serial}"
                )
            ]
